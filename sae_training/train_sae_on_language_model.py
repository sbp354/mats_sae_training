...
def get_recons_loss(sparse_autoencder, model, activation_store, num_batches=5):
    ...
    for _ in range(num_batches):
        ...
        recons_loss = model.run_with_hooks(
            batch_tokens,
            return_type="loss",
            fwd_hooks=[(hook_point, partial(replacement_hook, encoder=sparse_autoencder, model_cfg = model.cfg))],
        )
...
def replacement_hook(mlp_post, hook, encoder, model_cfg):
    hook_point = encoder.cfg.hook_point
    mlp_post_reconstr = encoder(mlp_post)[0]
    
    if str.find(hook_point, 'hook_z')>0:
        #print(mlp_post_reconstr.shape)
        batch_size = encoder.cfg.store_batch_size
        context_size = encoder.cfg.context_size
        n_heads = model_cfg.n_heads
        d_head = model_cfg.d_head
        mlp_post_reconstr = mlp_post_reconstr.reshape(batch_size, context_size, n_heads, d_head)
